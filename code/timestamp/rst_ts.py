...
def send_rst(syn_ack_packet):
    ...
    #以下几行可以比较timestamp的不同
    tcp_options = [('Timestamp', (0, 0))]
    #tcp_options = [('Timestamp', (int(time.time()), 0))]
    #tcp_options = [('Timestamp', (int(time.time())-2, ts_ecr))]
    #tcp_options = [('Timestamp', (int(time.time()), ts_ecr))]
    ...
    rst = TCP(
        ...
        #关键行，有 options 且 ts 为 0 才会丢包导致连接残留
	#不同的 options 可以用来测试timestamp 为 nop/和0等 
        options=tcp_options
        #options=[('MSS', 1460)]
        #options=[('MSS', 1460), ('Timestamp', (1732611916, 0))]
    )
